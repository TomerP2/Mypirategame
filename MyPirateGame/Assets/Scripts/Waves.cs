            ...
            for (int z = 0; z <= Dimension; z++)
            {
                ...
                for (int o = 0; o < Octaves.Length; o++)
                {
                    if (Octaves[o].alternate){
                        var perl = Mathf.PerlinNoise((x * Octaves[0].scale.x) / Dimension, (z * Octaves[0].scale.y) / Dimension) * Mathf.PI * 2f; 
                        y += Mathf.Cos(perl + Octaves[o].speed.magnitude * Time.time) * Octaves[o].height;
                    }
                    else
                    {
                        var perl = Mathf.PerlinNoise((x * Octaves[0].scale.x + Time.time * Octaves[0].speed.x) / Dimension, (z * Octaves[0].scale.y + Time.time * Octaves[0].speed.y) / Dimension) - 0.5f;
